...
pub fn prepare_login_request(req: &PrepareLoginRequest) -> anyhow::Result<Vec<u8>> {
  ...
  // TODO build pre login request
  // let payload = encode_protobuf(
	// 	libp2p::StateMessageRequest {
	// 		action: "user_pre_login".into(),
	// 		msg_b64: req_bytes,
	// 		uuid,
	// 	}
	// )?;
  // info!("user pre login payload => {:?}", payload);
...
// fn prepare_login(_uuid: &str, request: &PrepareLoginRequest) -> anyhow::Result<Vec<u8>> {
