...
export default {
  ...
  async mounted(){
    ...
    utils.register('home__consume', async (key, param)=>{
      await this.refreshList();   
    });
  },
  methods: {
    ...
    async refreshList(){
      ...
      this.list = _.map(list.nodes, (item)=>{
        const amount = this.showJSON({row:item}, 'tea_amount');
        item.income = _.toNumber(amount)*this.tapp.theta;
        return item;
      });
      this.loading(false);
    },
