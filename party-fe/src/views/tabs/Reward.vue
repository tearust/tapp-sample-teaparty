...
<div>
  ...
  >
    ...
    >
      <template slot-scope="scope">
        <span v-if="showJSON(scope, 'cml_id')!=='NA'">{{showJSON(scope, 'cml_id')}}</span>
      </template>
    </el-table-column>
    ...
    >
      <template slot-scope="scope">
        <span>{{showJSON(scope, 'reward')}}</span>
      </template>
    </el-table-column>
...
export default {
  ...
  async mounted(){
    ...
    utils.register('home__reward', async (key, param)=>{
      await this.refreshList();   
    });
  },
  methods: {
    ...
    async refreshList(){
      ...
      const list = await request.getLog({
        type: `equalTo: "event"`,
        name: `in: ["TAppConsumeRewardStatements"]`,
        tappId: `in: ["${this.id}"]`,
      });
